...
file_name <- "gsea_stable_path_hallmark.pdf"
...
plot <- gsea_dotplots(merged_df, surv_df = surv_df, gene_set = "hallmark",
                      title = "TCGA", file_name = file_names[1], n_path = NULL,
...
# plot km
file_names <- paste0(rep("figures/liver_km_", ncol(Z)), colnames(Z), "_test.pdf")

plots <- Map(function(factor, file_name) {
  factor <- setNames(factor, rownames(Z))
  surv_factor_km(surv = surv, factor = factor, model_label = "both", minprops = minprops)
