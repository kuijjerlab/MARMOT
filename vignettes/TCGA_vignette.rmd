...
for (cancer in cancers) {
  ...
  # plot bar plots
  p <- plot_data_dim(data = data, data_labels = c("no PCA", "PCA"),
                      log_x = FALSE, title = cancer)
...
for (i in cancers) {
  ...
  # plot
  p <- plot_fct_corr(df)
  ggsave(p, file = paste0("figures/", i, "_view_exclusion_both_abs.pdf"))
...
pdf(file = "gep_liver/liver_clin_ass_both_values.pdf", width = 11, height = 10)
