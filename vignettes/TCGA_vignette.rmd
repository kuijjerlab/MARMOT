...
for (i in cancers) {
  ...
  # plot
  p <- plot_fct_corr(df)
  ggsave(p, file = paste0("figures/", i, "_view_exclusion_both_abs.pdf"), width = 8)
...
load("gep_liver/gsea_stable_path_across_minprop.Rda")
...
file_name <- "gep_liver/figures/gsea_stable_path_hallmark.pdf"
...
plot <- gsea_dotplots(merged_df, surv_df = surv_df, gene_set = "hallmark",
                      ...
                      thresh = 1.30103, width = 140, height = 140, limitsize = FALSE)
...
common <- length(intersect(tcga$feature, gep$feature))
in_gep <- nrow(gep) - common
in_tcga <- nrow(tcga) - common
...
save(fisher_result, file = "outdeg_gep_tcga_fisher_result.Rda")
```
